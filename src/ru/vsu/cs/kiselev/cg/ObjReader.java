    ...
    public static Vector3f parseVertex(final ArrayList<String> wordsInLineWithoutToken, int lineInd) {
        try {
            if (wordsInLineWithoutToken.size() > 3) {
                throw new ObjReaderException("Большое количество: " + wordsInLineWithoutToken.size(), lineInd);
            }
            ...
            throw new ObjReaderException("Ошибка копирование значения", lineInd);

        } catch(IndexOutOfBoundsException e) {
            throw new ObjReaderException("Слишком много аргументов", lineInd);
        }
    }
